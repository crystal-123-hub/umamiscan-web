...
def norm_Adj(adjacency):
    ...
    adj_hat = adjacency + I
    D_hat = np.diag(np.power(np.array(adj_hat.sum(1)), -0.5).flatten())
    adj_Norm = adj_hat.dot(D_hat).transpose().dot(D_hat)
    return adj_Norm

def norm_fea(features):
    return features / features.sum(1).reshape(-1, 1)

def convert_to_graph_channel(seq: str, max_seq_len=20):
    ...
    for b in seq:
        if b not in aa_dict:
            print(f"Warning: Unknown amino acid '{b}', skipping...")
            continue
        seqSMILES.append(aa_dict[b])

    for i_, aminoAcidSMILES in enumerate(seqSMILES):
        aaMol = Chem.MolFromSmiles(aminoAcidSMILES)
        if aaMol is None:
            print(f"Error: Failed to parse SMILES for {aminoAcidSMILES} (index {i_})")
            continue
        ...
        if numAtoms > maxNumAtoms:
            print(f"Warning: Atom count {numAtoms} exceeds maxNumAtoms={maxNumAtoms}, skipping.")
            continue
    ...
    # 如果为空，返回全零张量
    if graphFeaturesOneSeq.size == 0:
        return np.zeros((max_seq_len, maxNumAtoms, 21), dtype=np.float32)
